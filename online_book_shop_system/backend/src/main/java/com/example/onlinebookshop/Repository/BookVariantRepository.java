...
public interface BookVariantRepository extends JpaRepository<BookVariant, Long> {
    ...
    @Query("SELECT v FROM BookVariant v JOIN FETCH v.book b WHERE b.id = :bookId AND v.deletedAt IS NULL AND b.deletedAt IS NULL AND v.isActive = true ORDER BY v.id")
    List<BookVariant> findActiveByBookId(@Param("bookId") Long bookId);
}
